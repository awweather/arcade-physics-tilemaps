...
interface ArcadeWorldConfig {
  ...
  /* @default 1   */
  timeScale?: number
  /* @default Sets Phaser   {@link Phaser.Physics.Arcade.World#gravity */
  gravity?: Vector2
  /* @default 0   */
  y?: number
  /* @default 0   */
  ...
  /* @default 0   */
  height?: number
  /* @default Sets Phaser   {@link Phaser.Physics.Arcade.World#checkCollision */
...
export class World extends EventEmitter {
  ...
  staticBodies: Set<StaticBody> = new Set()
  /** Static Bodies marked for deletion. */
  pendingDestroy: Set<Body | StaticBody> = new Set()
   ...
   */
  constructor(public scene, public config: ArcadeWorldConfig) {
    super()
  ...
  public addOverlap(
    ...
    const collider = new Collider(this, true, body1, body2, collideCallback, processCallback, callbackContext)
   ...
   */
  postUpdate() {
    let bodies: Set<Body | StaticBody> = this.bodies
    let len: number
    ...
    if (pending.size > 0) {
      ...
      for (const body of bodies) {
        if (body.physicsType === CONST.PHYSICS_TYPE.DYNAMIC_BODY) {
          ...
          dynamic.delete(body as Body)
        } else if (body.physicsType === CONST.PHYSICS_TYPE.STATIC_BODY) {
          staticTree.remove(body)
          staticBodies.delete(body as StaticBody)
        }
   ...
   * @return {boolean} True if separation occurred, otherwise false.
   */
  separateCircle(body1: Body | StaticBody, body2: Body | StaticBody, overlapOnly, bias?) {
    ...
    if (body1.isCircle !== body2.isCircle) {
      ...
      overlap =
        body1.halfWidth +
        body2.halfWidth -
        DistanceBetween(body1.center.x, body1.center.y, body2.center.x, body2.center.y)
    }
    ...
    let p =
      (2 * (body1.velocity.x * nx + body1.velocity.y * ny - body2.velocity.x * nx - body2.velocity.y * ny)) /
    ...
    const angle = AngleBetweenPoints(body1.center, body2.center)
    const overlapX = (overlap + MATH_CONST.EPSILON) * Math.cos(angle)
    const overlapY = (overlap + MATH_CONST.EPSILON) * Math.sin(angle)
   ...
   */
  intersects(body1: Body | StaticBody, body2: Body | StaticBody) {
